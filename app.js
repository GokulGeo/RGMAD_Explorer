...
function generateThumbnailFromL4(name, imgElement, placeholderElement) {
    ...
    // First, get the geometry of the l4 tile - use fresh query object
...
const performSearch = debounce((text) => {
    ...
    // Filter to only -l4 tiles to avoid duplicates
    const whereClause = `(UPPER(Name) LIKE UPPER('%${sqlSafeText}%') OR UPPER(TITLE) LIKE UPPER('%${sqlSafeText}%')) AND Name LIKE '%-l4'`;
    
    // Create a fresh query object to search ALL records (not limited by map extent)
    L.esri.query({ url: urls.footprints + '/0' })
        .where(whereClause)
        .returnGeometry(true)
...
function renderVisibleFeatures() {
    ...
    const allFields = Object.keys(allLoadedFeatures[0].properties);
...
function generateTableHeaders(feature) {
    const thead = document.getElementById('table-head');
    const allFields = Object.keys(feature.properties);
    ...
    fields.forEach(field => {
        ...
        th.addEventListener('click', () => sortTable(field, th));
...
function finalizeLoading() {
    ...
    if (uniqueFeatures.length > 0) {
         const allFields = Object.keys(uniqueFeatures[0].properties);
         const excludeFields = [
             'OBJECTID', 'MinPS', 'MaxPS', 'LowPS', 'HighPS', 
             'Category', 'CenterX', 'CenterY', 'ZOrder', 'TypeID', 
             'ItemTS', 'UriHash', 'Shape_Leng', 'Shape_Area', 'Shape_Length',
         ...
         function renderChunk() {
             ...
             if (i < uniqueFeatures.length) {
                 ...
                 countSpan.textContent = `(Rendering ${i}/${uniqueFeatures.length} unique sheets...)`;
             } else {
                 countSpan.textContent = `(${uniqueFeatures.length} unique sheets, ${totalRecordsLoaded} total tiles)`;
             }
         }
         renderChunk();
    } else {
